...
#default data= 'ZxpLVPJD43PjTJsBma4YpmdqLFF8LcMJv4q2fXH/SeaRFg==' 
...
def decrypt(data, key):
    '''decode from base64, decrypt, check magic string (password worked), 
       and return without magic string
    returns None if password did not work'''
    data = convert(data.decode('base64'), key)
    if data.startswith(magic_string):
        return data[len(magic_string):]

def get(key):
    '''decode global variable "data" with given key'''
...
def save(new_data, key, new_geo=None):
    ...
    global data #technically not needed in GUI mode
    fn = __file__
    if fn.endswith('.pyc'):
...
class PasswordDialog(ttk.Frame):
    ...
    def store_pass(self, event=None):
        ...
        self.parent.secret = get(self.parent.key)
...
def main():
    ...
    root.geometry(geometry or "400x500+300+300")
...
####dammed data####
geometry = None
data='ZxpLVPJD43PjTJsBma4YpmdqLFF8LcMJv4q2fXH/SeaRFg=='
####dammed data####
